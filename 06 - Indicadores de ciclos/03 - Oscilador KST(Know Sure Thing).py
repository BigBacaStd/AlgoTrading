...
def Know_Sure_Thing(df: pd.DataFrame, roclen1: int = 10, roclen2: int = 15, roclen3: int = 20, roclen4: int = 30, smalen1: int = 10,
    ...
    # ROCs con Medias Moviles
    ROC1 = price_col.diff(periods=roclen1) / price_col.shift(periods=roclen1) * 100
    ROC1 = ROC1.rolling(window=smalen1, min_periods=smalen1).mean()
    ROC2 = price_col.diff(periods=roclen2) / price_col.shift(periods=roclen2) * 100
    ROC2 = ROC2.rolling(window=smalen2, min_periods=smalen2).mean()
    ROC3 = price_col.diff(periods=roclen3) / price_col.shift(periods=roclen3) * 100
    ROC3 = ROC1.rolling(window=smalen3, min_periods=smalen3).mean()
    ROC4 = price_col.diff(periods=roclen4) / price_col.shift(periods=roclen4) * 100
    ROC4 = ROC1.rolling(window=smalen4, min_periods=smalen4).mean()

    # KST

    KST_df = (ROC1 + 2 * ROC2 + 3 * ROC3 + 4 * ROC4)
...
df["Cruces"] = np.where(
    (df["KST"] > df["KST Signal"]) & (df["KST"].shift(periods=1) < df["KST Signal"].shift(periods=1)), 1,
...
for ax in axes:
    ax.yaxis.label.set_size(18) # Cambiar el tamaño segun sea necesario
