...
def Bandas_Bollingar(df: pd.DataFrame, longitud: int = 20, std_dev: float = 2.0, ddof: int = 0,
...
ticker = "BTC-USD"
df = yf.download(ticker, start="2024-01-01", end="2025-10-30", multi_level_index=False)

# Calcular Indicador

bb_df = Bandas_Bollingar(df, longitud=20, std_dev=2.0)

# Crear el grafico

add_plots = [
    mpf.make_addplot(bb_df["BB_Up"], color="red", linestyle="--", label="Banda Superior"),
    mpf.make_addplot(bb_df["BB_Down"], color="blue", linestyle="--", label="Banda Inferior"),
    mpf.make_addplot(bb_df["MA"], color="green", label="Media Movil")
]
...
ax[0].set_xlabel("Bandas de Bollinger", fontsize=16, weight="bold")
