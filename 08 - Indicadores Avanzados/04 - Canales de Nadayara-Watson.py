...
def Nadaraya_Watson_Envelope(df: pd.DataFrame, longitud: int = 500, ancho_banda: float = 8.0, factor: float = 3.0,
    ...
    assert df.shape[0] >= longitud, "La longitud del DataFrame debe ser mayor o igual a la longitud especificada"
    ...
    # Calcular el estimador aplicando pesos
    ...
    precios_columna_desplazado = precios_columna.shift(periods=1)
    direccion_bandas = np.where(

        ((precios_columna_desplazado < (estimador_y2 - mae))) & ((precios_columna > (estimador_y2 - mae))), 1,
        np.where(((precios_columna_desplazado > (estimador_y2 + mae))) & ((precios_columna < (estimador_y2 + mae))), -1,
                 np.nan)
    )
...
ticker = "BCH-USD"
df = yf.download(ticker, start="2023-01-01", end="2025-11-22", interval="1d", multi_level_index=False)
...
nwe_plots = [
    ...
    mpf.make_addplot(df["High"].where(df.index.isin(nwe_bajista.index), np.nan)[-500:] * 1.01, label="Señal Bajista",
                     color="red",
                     type="scatter", marker="v", markersize=100),
    mpf.make_addplot(df["Low"].where(df.index.isin(nwe_alcista.index), np.nan)[-500:] * 0.99, label="Señal Alcista",
                     color="green",
