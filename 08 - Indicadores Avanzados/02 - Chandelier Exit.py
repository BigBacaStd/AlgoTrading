...
def Chandelier_Exit(df: pd.DataFrame, CE_longitud: int = 22, ATR_longitud: int = 22,
                    multiplicador: float = 3.0) -> pd.DataFrame:
    """
...
ticker = "BCH-USD"
df = yf.download(ticker, start="2025-01-01", end="2025-11-23", multi_level_index=False)
...
addplots = [
    ...
    mpf.make_addplot(ce["Chand_Exit_Long"].where(ce["dir"] == 1, np.nan), panel=0, color="green", width=3,
                     label="Chandelier Exit Long"),
    mpf.make_addplot(ce["Chand_Exit_Short"].where(ce["dir"] == -1, np.nan), panel=0, color="red", width=3,
                     label="Chandelier Exit Short"),
    mpf.make_addplot(ce["Chand_Exit_Long"].where(df["High"].index.isin(señal_alcista.index.tolist()), np.nan) * 0.97,
                     panel=0, color="green",
                     marker="^", markersize=100, type="scatter"),
    mpf.make_addplot(ce["Chand_Exit_Short"].where(df["Low"].index.isin(señal_bajista.index.tolist()), np.nan) * 1.03,
                     panel=0, color="red",
                     marker="v", markersize=100, type="scatter")

]

mpf.plot(df, type="candle", style="yahoo", volume=True, addplot=addplots, figsize=(25, 10),
         title="Precios, Volumen e Indicador Chandelier Exit",
