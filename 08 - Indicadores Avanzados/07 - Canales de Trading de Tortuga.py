...
def Canales_Trading_Tortuga(df: pd.DataFrame, longitud_entrada: int = 20, longitud_salida: int = 10) -> pd.DataFrame:
    ...
    señal_compra = (High == entrada_high.shift(periods=1)) | \
                   (np.where(
                       (High > entrada_high.shift(periods=1)) & (High.shift(periods=1) < entrada_high.shift(periods=2)),
                       True, False))
    señal_venta = (Low == entrada_low.shift(periods=1)) | \
                  (np.where(
                      (entrada_low.shift(periods=1) > Low) & (entrada_low.shift(periods=2) < Low.shift(periods=2)),
                      True, False))
    salida_compra = (Low == salida_low.shift(periods=1)) | \
                    (np.where(
                        (salida_low.shift(periods=1) > Low) & (salida_low.shift(periods=2) < Low.shift(periods=2)),
                        True, False))
    salida_venta = (High == salida_high.shift(periods=1)) | \
                   (np.where(
                       (High > salida_high.shift(periods=1)) & (High.shift(periods=1) < salida_high.shift(periods=2)),
                       True, False))
    ...
    TDC["señal_compra"] = np.where(señal_compra & (señales_df["O3"] < señales_df["O1"].shift(periods=1)), entrada_low,
                                   np.nan)
    TDC["señal_venta"] = np.where(señal_venta & (señales_df["O4"] < señales_df["O2"].shift(periods=1)), entrada_high,
                                  np.nan)
    TDC["salida_compra"] = np.where(salida_compra & (señales_df["O1"] < señales_df["O3"].shift(periods=1)),
                                    entrada_high, np.nan)
    TDC["salida_venta"] = np.where(salida_venta & (señales_df["O2"] < señales_df["O4"].shift(periods=1)), entrada_high,
                                   np.nan)
...
ticker = "BCH-USD"
df = yf.download(ticker, start="2024-01-01", end="2025-11-23", multi_level_index=False)
...
graficos_adicionales = [
    ...
    mpf.make_addplot(df["High"].where(tdc.index.isin(compras_copia.index)) * 1.03, type="scatter", marker="^",
                     markersize=100,
                     color="green", label="Entrada de Señales de Compra"),
    mpf.make_addplot(df["High"].where(tdc.index.isin(compras_copia["segundaFecha"])) * 1.03, type="scatter", marker="v",
                     markersize=100,
                     color="green", label="Salida de Señales de Compra"),
    mpf.make_addplot(df["Low"].where(tdc.index.isin(ventas_copia.index)) * 0.97, type="scatter", marker="^",
                     markersize=100,
                     color="red", label="Entrada de Señales de Venta"),
    mpf.make_addplot(df["Low"].where(tdc.index.isin(ventas_copia["segundaFecha"])) * 0.97, type="scatter", marker="v",
                     markersize=100,
                     color="red", label="Salida de Señales de Venta")

]

fig, ax = mpf.plot(df, type="candle", style="yahoo", volume=True, figsize=(22, 10), figscale=3.0,
                   addplot=graficos_adicionales, returnfig=True)
plt.show()
