...
def Squeeze_Momentum(df: pd.DataFrame, longitud_bb: int = 20, desviacion_std_bb: float = 2.0, longitud_kc: int = 20,
                     multiplicador_kc: float = 1.5,
    ...
    squeeze = df["Close"].diff(periods=periodos_momentum).rolling(window=longitud_momentum,
                                                                  min_periods=longitud_momentum).mean()
...
# Recordatorio:
#   - El Indicador de SM es eficaz para detectar períodos en los que el mercado están en compresión o baja volatilidad (SQZ_ON). Estos períodos
