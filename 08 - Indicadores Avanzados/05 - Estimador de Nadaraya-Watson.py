...
def Estimador_Nadaraya_Watson(df: pd.DataFrame, longitud: int = 500, ancho_banda: float = 8.0, factor: float = 3.0,
    ...
    # Calcular Indicador
    assert df.shape[0] >= longitud, "La longitud de los datos debe ser >= longitud"
    ...
    # Asignar etiquetas de dirección en la base a los cambios
...
ticker = "BCH-USD"
datos = yf.download(ticker, start="2022-01-01", end="2025-11-23", interval="1d", multi_level_index=False)
...
graficos_nwe = [
    ...
    mpf.make_addplot(nwe_cambio_alcista, label="Señal Alcista", type="scatter", color="green", marker="^",
                     markersize=100),
    mpf.make_addplot(nwe_cambio_bajista, label="Señal Bajista", type="scatter", color="red", marker="v", markersize=100)

]

fig, ax = mpf.plot(datos[-500:], type="candle", style="yahoo", volume=True, figsize=(22, 10), addplot=graficos_nwe,
                   returnfig=True)
ax[0].legend(loc="lower left", fontsize=12)
plt.show()

# Recordatorio: - El Estimador NW es un método de estimación no paramétrica. Se utiliza para suavizar datos y hacer
#  predicciones sin asumir una forma específica para la función subyacente que genera los datos. En lugar de basarse
#  en un modelo paramétrico con un número fijo de parámetros, el estimador de NW calcula una serie de promedios
#  ponderados utilizando un kernel, lo que le permite adaptarse de manera flexible a la estructura de los datos. Esto
#  lo hace particularmente útil para situaciones en las que se desea capturar patrones de datos ruidosos o con
