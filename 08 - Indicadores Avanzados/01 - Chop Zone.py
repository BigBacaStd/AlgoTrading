...
def Chop_Zone(df: pd.DataFrame, longitud: int = 30, longitud_ema: int = 34, columna: str = "Close") -> pd.Series:
    ...
    Cómo Operarlo:
        ...
        El indicador Chop Zone es utilizado para identificar la volatilidad y la fuerza de una tendencia en los mercados
                                    ...
                                    np.where((angulo_ema1 >= 0.71) & (angulo_ema1 < 2.14), 3,
                                             np.where(angulo_ema1 <= -5, 4,
                                                      np.where((angulo_ema1 <= -3.57) & (angulo_ema1 > -5), 5,
                                                               np.where((angulo_ema1 <= -2.14) & (angulo_ema1 > -3.57),
                                                                        6,
                                                                        np.where((angulo_ema1 <= -0.71) & (
                                                                                    angulo_ema1 > -2.14), 7, 8))))))))
...
# Obtener Datos
ticker = "BCH-USD"
df = yf.download(ticker, start="2025-01-01", end="2025-11-22", interval="1d", multi_level_index=False)
...
df["Close"].loc[cz.index].reset_index(drop=True).plot(ax=ax[0], xlabel="Fecha", ylabel="Precio",
...
ax[1].set_title("Indicador Chop Zone", fontsize=20)
