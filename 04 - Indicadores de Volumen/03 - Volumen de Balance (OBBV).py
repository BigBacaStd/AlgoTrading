...
def On_Balance_Volume(df: pd.DataFrame, columna: str = "Close") -> pd.Series:
    ...
    cuando la volatilidad es alta. Ademas, se puede ajustar en una Media Móvil al cálculo y comprar cuando la MA cruce por encima
...
ticker = "BTC-USD"
df = yf.download(ticker, start="2022-01-01", end="2025-10-23", interval="1d", multi_level_index=False)
...
ax2.text(df.index[-1] + timedelta(days=17), obv.max() * 0.8, "Señal de Compra cuando \nOBV cruza por encima de SMA", fontsize=12, color="lime",
         bbox=dict(facecolor="white", edgecolor="lime", boxstyle="round, pad=0.5"))
ax2.text(df.index[-1] + timedelta(days=17), obv.min() * 0.8, "Señal de Venta cuando \nOBV cruza por encima de SMA", fontsize=12, color="red",
...
# - El OBV es una herramienta que utiliza el volumen para evaluar la fuerza y la confirmación de una tendencia en los precios.
