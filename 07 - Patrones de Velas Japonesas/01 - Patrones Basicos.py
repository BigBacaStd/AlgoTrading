...
def is_hammer(df: pd.DataFrame):
    ...
    # Verificar que la vela sea más baja en las últimas 3 velas
...
def is_hanging_man(df: pd.DataFrame):
    ...
    # Verificar que la vela sea más alta en las últimas 3 velas
...
hangin_man = is_hanging_man(df)
print(hangin_man)

# Graficar Patrones

mpf.plot(df.loc[hangin_man[hangin_man].index], style="yahoo", type="candle", title="Hanging Man", figsize=(22, 5), figscale=3.0)
plt.show()

# Patron No. 3: Doji
# El Doji ocurre cuando el precio de apertura y cierrre son casi iguales, lo que sugiere indecision en el mercado.
